...
for index in range(len(dict['x'])):
  ...
  path = "C:/Users/Administrator/Desktop/lambai/11_NR/results/adam/cell.pkl"
  with open(path, 'rb') as f:
    dict = pickle.load(f)
  dict['x'] = np.concatenate((np.array(dict['x']), np.expand_dims(
    2 * np.array(dict['center'])[:, 0], axis=-1) - np.array(dict['x'])),
                             axis=1)
  dict['y'] = np.concatenate((np.array(dict['y']), np.expand_dims(
    2 * np.array(dict['center'])[:, 1], axis=-1) - np.array(dict['y'])),
                             axis=1)
  dict['z'] = np.concatenate((np.array(dict['z']), - np.array(dict['z'])),
                             axis=1)
  alpha = 1
  for index in range(len(dict['x'])):
    ...
    for vert in mesh.vertices:
      vert.co.x = vert.co.x * dict['radii'][0]
      vert.co.y = vert.co.y * dict['radii'][1]
      vert.co.z = vert.co.z * dict['radii'][2]
      after_rotation = np.array([vert.co.x, vert.co.y, vert.co.z]) @ dict[
        'ellipse_direction'] @ dict['r'][index].T
      vert.co.x = after_rotation[0]
      vert.co.y = after_rotation[1]
      vert.co.z = after_rotation[2]
    ...
    for i, verts in enumerate(nearest):
      base = verts[0].co
      far = verts[len(verts) - 1].co
      offset = [(x[i] - center[0]) - base.x,
                (y[i] - center[1]) - base.y,
                (z[i] - 0) - base.z]
      longest = np.linalg.norm(
        [far.x - base.x, far.y - base.y, far.z - base.z])
      for j, vert in enumerate(verts):
        ...
        vert.co.x += alpha * gaussian(l, 0, longest) * offset[0]
        vert.co.y += alpha * gaussian(l, 0, longest) * offset[1]
        vert.co.z += alpha * gaussian(l, 0, longest) * offset[2]
    for i, verts in enumerate(nearest):
