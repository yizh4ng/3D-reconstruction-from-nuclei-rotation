...
def get_axis_eigen(cell, axis=np.array([1, 0, 0]), verbose=False):
  rotation = []
...
if __name__ == '__main__':
  ...
  def get_all_eigen(path):
    pkl_paths = finder.walk(path, pattern='*_3d.pkl', recursive=True)
    print(len(pkl_paths))
    eigens = []
    for path in pkl_paths:
      ...
      if eigen is not None:
        eigens.append(get_axis_eigen(pkl, verbose=False))
