...
# A and b are N * 2 size tenors out put the sum of distances of each pairs
def distance(A):
  dis = tf.reduce_sum(
    # tf.sqrt(tf.maximum(
    tf.reduce_sum((tf.expand_dims(A, 1)-tf.expand_dims(A, 0))**2, 2)
    #, 1e-10 ))
  )

  return dis

def loss_function(points, num_frames):
  loss = 0
  for t in range(0, num_frames - 1):
    loss += tf.math.abs(distance(points[t]) - distance(points[t+1]))
                #0.1 * tf.norm(z[t] - z[t + 1])
  return loss

def loss_function_evaluate(points, num_frames):
  loss = 0
  for t in range(0, num_frames - 2):
    loss += tf.math.abs(
      distance(tf.slice(points, [t, 0, 0], [t,-1,-1]),tf.slice(points, [t+1, 0, 0], [t+1,-1,-1])))
  return loss
...
if __name__ == '__main__':
  ...
  sess = tf.Session()
  loss = loss_function(points,num_frames)
